...
def api_data():
                    ...
                    if name in active_names:
                        ...
                        for key, val in row.items():
                            if "Volume Weighted Average Price" in key or "VWAP" in key:
                                if val: data_map[name][f"{tf}_vwap"] = val
    ...
    for name in active_names:
        ...
        # Real Confirmation Logic using VWAP
        conf = "WAITING"
        ...
        try:
            if p5 != '...' and p15 != '...':
                ...
                # If we have VWAPs, use them for a hard confirmation
    ...
    return {"html": html or '<tr><td colspan="5" style="text-align:center;">[ NO DATA ]</td></tr>'}
...
def stop():
    ...
    if stopped > 0:
        flash(f"Successfully Stopped {stopped} Engines", "success")
    else:
        flash("All Engines were already Stopped", "error")
    return redirect(url_for('index'))
