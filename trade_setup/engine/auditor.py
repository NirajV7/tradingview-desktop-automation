    ...
    for symbol in active_symbols:
        ...
        if now_time >= datetime.strptime("15:15:00", "%H:%M:%S").time():
            ...
            if self.dry_run:
                ...
                self.log_trade_to_journal(symbol, entry, current_price, qty, direction="BUY", exit_reason="3:15 PM MANDATORY SQUARE-OFF")
                ...
                try:
                    ...
                    self.log_trade_to_journal(symbol, entry, current_price, qty, direction="BUY", exit_reason="3:15 PM MANDATORY SQUARE-OFF", exit_order_id=exit_order_id)
                    self.completed_trades_today.add(symbol)
                    del self.active_trades[symbol]
                except Exception as e:
