...
def place_kite_sell_bracket_defense(self, symbol, quantity, entry_price, stop_loss, target):
    ...
    try:
        ...
        # 2. Wait and check if SELL order is fully filled/COMPLETE
        filled = False
        for attempt in range(20):  # Wait up to 10 seconds (20 * 0.5s)
            time.sleep(0.5)
            try:
                ...
                if history:
                    status = history[-1].get("status")
                    print(f"   [Order Check] SELL Order {entry_order_id} status: {status}")
                    if status == "COMPLETE":
                        filled = True
                        break
                    elif status in ["REJECTED", "CANCELLED"]:
        ...
        # 3. Place Stop Loss Pending Order (Standard SL Limit) with 3 retries
        print(f"🛡️ Placing Pending BUY SL Stop Loss Order at Trigger: ₹{stop_loss:.2f}...")
        sl_order_id = None
        for sl_attempt in range(3):
            try:
                sl_order_id = self.kite.place_order(
                ...
                if sl_attempt < 2:
                    ...
                    # 3 retries exhausted -> Emergency Square-Off (Market buy order to cover the short)
                    print(f"🚨 CRITICAL: Failed to place Stop Loss order after 3 retries!")
                    print(f"🛒 Initiating EMERGENCY MARKET SQUARE-OFF to protect capital...")
                    try:
                        exit_order_id = self.kite.place_order(
